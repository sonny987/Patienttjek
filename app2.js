...
// ── AFDELING OPSTART ──
// Tjek om der er en gemt session — vis ellers login
(function deptStartup(){
  if(loadDeptSession()){
    // Session fundet — vis app direkte
    hideDeptLogin();
    if(_isAdmin) renderAdminDeptHeader(); else renderDeptHeader();
  } else {
    // Ingen session — vis login skærm, stop sync til login
    showDeptLogin();
  }
})();
...
function renderCockpit(){
  ...
  // Find alerts på tværs af patienter
  const alerts = [];
  patients.forEach(p=>{
    const allTasks = getAllTasks(p);
    // Mangler TOKS
    const toks = p.sections.ankomst?.items?.find(x=>x.label==='TOKS');
    if(toks && !toks.done) alerts.push({type:'toks', pid:p.id, name:p.name, room:p.room, team:p.team, msg:'TOKS ikke udført'});
    // Medicin ikke givet
    (p.sections.medicin?.items||[]).forEach(med=>{
      ...
      if(overdue.length) alerts.push({type:'med', pid:p.id, name:p.name, room:p.room, team:p.team, msg:`${med.name||'Medicin'} ikke givet (${overdue.length} tider)`});
    });
    // P1 opgaver
    (p.sections.prioritering?.items||[]).filter(x=>!x.done&&x.priority==='red').forEach(item=>{
      alerts.push({type:'p1', pid:p.id, name:p.name, room:p.room, team:p.team, msg:item.label});
    });
  });
  ...
  if(alerts.length){
    html += `<div style="margin-bottom:18px">
      <div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--red);margin-bottom:8px;letter-spacing:.5px">🚨 KRÆVER OPMÆRKSOMHED (${alerts.length})</div>`;
    const typeIcons = {toks:'🩺',med:'💊',p1:'⚡'};
    const typeBg = {toks:'var(--ybg)',med:'var(--obg)',p1:'var(--rbg)'};
    const typeColor = {toks:'var(--yellow)',med:'var(--orange)',p1:'var(--red)'};
    alerts.forEach(a=>{
      html += `<div style="background:${typeBg[a.type]};border-radius:8px;padding:7px 12px;margin-bottom:5px;display:flex;align-items:center;gap:10px;cursor:pointer" onclick="closeCockpit();setActivePatient('${a.pid}')">
        <span style="font-size:14px">${typeIcons[a.type]}</span>
        <div style="flex:1">
          <span style="font-size:12px;font-weight:700;color:${typeColor[a.type]}">${a.msg}</span>
          <span style="font-size:11px;color:var(--muted);margin-left:8px">${a.name} · ${a.room||'?'}</span>
        </div>
    ...
    html += `<div style="background:var(--gbg);border:1px solid var(--gbrd);border-radius:10px;padding:12px;margin-bottom:18px;text-align:center;color:var(--green);font-weight:700">✓ Ingen akutte alerts — alt ser fint ud!</div>`;
  }

  // Patientoversigt tabel
  html += `<div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--muted);margin-bottom:10px;letter-spacing:.5px">🏥 ALLE PATIENTER</div>
  ...
  patients.forEach(p=>{
    ...
    html += `<div style="background:var(--s2);border:1px solid var(--border);border-radius:10px;padding:10px 14px;cursor:pointer" onclick="closeCockpit();setActivePatient('${p.id}')">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap">
        ...
        ${p1cnt?`<span style="font-size:10px;font-weight:800;background:var(--rbg);color:var(--red);border-radius:4px;padding:1px 6px;margin-left:auto">🚨 ${p1cnt} P1</span>`:prios.length?`<span style="font-size:10px;font-weight:800;background:var(--ybg);color:var(--yellow);border-radius:4px;padding:1px 6px;margin-left:auto">⚡ ${prios.length}</span>`:'<span style="margin-left:auto"></span>'}
      </div>
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="font-size:11px;font-weight:700;color:${barColor};min-width:36px;text-align:right">${done}/${total}</span>
      </div>
    </div>`;
  });
  html += '</div>';
