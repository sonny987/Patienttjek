...
function addPatient(){
  ...
  try {
    ...
    patients.push({id:uid(),name:displayName,room,diag,team,open:false,customizeOpen:false,sections:secs});
    save();
  } catch(e) {
...
function removePt(id){
  ...
  if(_fbDb && p){
    fetch(_fbDb.url(_deptPath('archive/'+id)), {
...
let modalSecState = {};
let _deptDefaults = null; // Afdelingens gemte standardindstillinger

// Hent afdelingens standardindstillinger fra Firebase
async function loadDeptDefaults(){
  if(!_currentDeptId) return;
  try {
    const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
    const res = await fetch(`${base}/system/deptDefaults/${_currentDeptSecret||_currentDeptId}.json`);
    if(res.ok){ const d = await res.json(); if(d) _deptDefaults = d; }
  } catch(e){ console.warn('loadDeptDefaults fejl:', e); }
}

// Gem afdelingens standardindstillinger til Firebase (kun admin)
async function saveDeptDefaults(){
  const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
  await fetch(`${base}/system/deptDefaults/${_currentDeptSecret||_currentDeptId}.json`, {
    method:'PUT', headers:{'Content-Type':'application/json'},
    body: JSON.stringify(modalSecState)
  });
  _deptDefaults = JSON.parse(JSON.stringify(modalSecState));
  showPrioToast('Standardindstillinger gemt for afdelingen ✓','green');
}

function initModalState(){
  modalSecState = {};
  MODAL_SEC_DEFS.forEach(sd=>{
    // Brug afdelingens gemte standarder hvis de findes — ellers ALT FRAVALGT
    if(_deptDefaults && _deptDefaults[sd.key]){
      modalSecState[sd.key] = JSON.parse(JSON.stringify(_deptDefaults[sd.key]));
    } else {
      modalSecState[sd.key] = {
        enabled: false, // ALT FRAVALGT som standard
        tasks: Object.fromEntries(sd.tasks.map(t=>[t,false]))
      };
    }
  });
}
...
function renderModalSections(){
  ...
  // Admin-knapper øverst
  const adminBar = _isAdmin ? `
    <div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap;align-items:center;background:var(--ybg);border:1px solid var(--ybrd);border-radius:8px;padding:8px 10px">
      <span style="font-size:11px;font-weight:700;color:var(--yellow)">🔧 Admin:</span>
      <button class="btn btn-ghost btn-sm" style="font-size:11px;color:var(--yellow);border-color:var(--yellow)" onclick="saveDeptDefaults()">💾 Gem som standard for afdelingen</button>
      <button class="btn btn-ghost btn-sm" style="font-size:11px" onclick="modalSelectAll(true)">✓ Vælg alle</button>
      <button class="btn btn-ghost btn-sm" style="font-size:11px" onclick="modalSelectAll(false)">✕ Fravælg alle</button>
    </div>` : `
  ...
  el.innerHTML = adminBar + MODAL_SEC_DEFS.map(sd=>{
    const st = modalSecState[sd.key];
    const en = st.enabled;
    const hasTasks = sd.tasks.length > 0;
    return `
    <div style="background:var(--s2);border:1px solid ${en?'var(--blue)':'var(--border)'};border-radius:10px;padding:10px 12px;transition:border-color .15s">
...
function modalSelectAll(val){
  MODAL_SEC_DEFS.forEach(sd=>{
    modalSecState[sd.key].enabled = val;
    sd.tasks.forEach(t=>{ modalSecState[sd.key].tasks[t] = val; });
  });
  renderModalSections();
}

function toggleModalSec(key,val){
  modalSecState[key].enabled=val;
  // Når sektion slås til, slå alle under-opgaver til også
  if(val) MODAL_SEC_DEFS.find(s=>s.key===key)?.tasks.forEach(t=>{ modalSecState[key].tasks[t]=true; });
  renderModalSections();
}
...
// ── AFDELING OPSTART ──
(function deptStartup(){
  if(loadDeptSession()){
    hideDeptLogin();
    if(_isAdmin){
      renderAdminDeptHeader();
      if(!_currentDeptId){
        openAdminDeptPanel();
      } else {
        startSync();
...
if('serviceWorker' in navigator){
  navigator.serviceWorker.register('/Patienttjek/sw.js')
...
window.addEventListener('beforeinstallprompt', e => {
  e.preventDefault();
  _installPrompt = e;
  // Vis install-knap i headeren
  const btn = document.getElementById('pwaInstallBtn');
  if(btn) btn.style.display = '';
});

// Skjul knap når app er installeret
...
function installPWA(){
  // iOS: vis instruktioner da iOS ikke understøtter beforeinstallprompt
  const isIOS = /iphone|ipad|ipod/i.test(navigator.userAgent) && !window.MSStream;
  const isStandalone = window.navigator.standalone || window.matchMedia('(display-mode: standalone)').matches;

  if(isStandalone){
    showPrioToast('Appen er allerede installeret!', 'yellow');
    return;
  }

  if(isIOS){
    // Vis iOS-instruktioner
    const d = document.createElement('div');
    d.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:flex-end;justify-content:center`;
    d.innerHTML = `<div style="background:var(--s1);border-radius:16px 16px 0 0;padding:24px 20px;max-width:400px;width:100%;margin-bottom:0">
      <div style="font-size:18px;font-weight:800;margin-bottom:12px;text-align:center">📱 Installer på iPhone/iPad</div>
      <div style="font-size:13px;color:var(--muted2);line-height:1.8">
        <div style="margin-bottom:8px">1. Tryk på <strong>Del-ikonet</strong> <span style="font-size:16px">⬆️</span> i Safari</div>
        <div style="margin-bottom:8px">2. Scroll ned og tryk <strong>"Føj til hjemmeskærm"</strong></div>
        <div>3. Tryk <strong>"Tilføj"</strong> øverst til højre</div>
      </div>
      <button onclick="this.closest('div[style*=inset]').remove()" 
        style="width:100%;margin-top:18px;padding:12px;background:var(--blue);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit">
        Forstået
      </button>
    </div>`;
  ...
  if(_installPrompt){
    ...
    showPrioToast('Åbn siden i Chrome eller Edge for at installere', 'yellow');
  }
}

// Vis altid install-knap på iOS (da beforeinstallprompt ikke fyres)
...
function openVagtovl(){
  document.getElementById('vagtovlOverlay').style.display = 'flex';
  document.getElementById('vagtovlDato').textContent =
    new Date().toLocaleDateString('da-DK',{weekday:'long',day:'numeric',month:'long',hour:'2-digit',minute:'2-digit'});
  // Restore saved note
  const saved = localStorage.getItem('kp2_vagtnote') || '';
  const noteEl = document.getElementById('vagtovlNote');
  if(noteEl) noteEl.value = saved;
  setVagtTeam('Alle');
  setVagtType('dag');
}

function closeVagtovl(){
  document.getElementById('vagtovlOverlay').style.display = 'none';
}

function setVagtType(type){
  _vagtType = type;
  ['dag','aften','nat'].forEach(t=>{
    const b = document.getElementById('vagtBtn-'+t);
    if(b) b.style.opacity = t===type ? '1' : '0.45';
  });
  renderVagtovl();
}

function setVagtTeam(team){
  _vagtTeam = team;
  ['Alle','A','B','C','Obs10','Obs5'].forEach(t=>{
    const b = document.getElementById('vagtTeamBtn-'+t);
    if(!b) return;
    b.style.background = t===team ? 'var(--bbg)' : '';
    b.style.borderColor = t===team ? 'var(--blue)' : '';
    b.style.color = t===team ? 'var(--blue)' : '';
  });
  renderVagtovl();
}

function renderVagtovl(){
  const container = document.getElementById('vagtovlContent');
  if(!container) return;
  const pts = _vagtTeam==='Alle' ? patients : patients.filter(p=>p.team===_vagtTeam);
  if(!pts.length){ container.innerHTML = '<div style="color:var(--muted);padding:20px;text-align:center">Ingen patienter</div>'; return; }

  const allPrios = [];
  pts.forEach(p=>(p.sections.prioritering?.items||[]).filter(x=>!x.done).forEach(item=>{
    allPrios.push({...item, ptName:p.name, ptRoom:p.room, team:p.team});
  }));
  ...
  if(allPrios.length){
    html += `<div style="margin-bottom:14px">
      <div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--muted);margin-bottom:8px;letter-spacing:.5px">⚡ ÅBNE PRIORITETER (${allPrios.length})</div>`;
    allPrios.sort((a,b)=>a.priority==='red'?-1:1).forEach(item=>{
      const c = item.priority==='red' ? 'var(--red)' : 'var(--yellow)';
      const bg = item.priority==='red' ? 'var(--rbg)' : 'var(--ybg)';
      html += `<div style="background:${bg};border-radius:8px;padding:8px 12px;margin-bottom:5px;display:flex;align-items:center;gap:10px">
        <span style="font-size:10px;font-weight:800;color:#fff;background:${c};padding:1px 6px;border-radius:4px">${item.priority==='red'?'P1':'P2'}</span>
        <span style="flex:1;font-size:12px;font-weight:600">${item.label}</span>
        <span style="font-size:10px;color:var(--muted)">${item.ptName}${item.ptRoom?' · '+item.ptRoom:''}</span>
      </div>`;
    });
    html += '</div>';
  }
  ...
  // Patientoversigt
  html += `<div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--muted);margin-bottom:8px;letter-spacing:.5px">🏥 PATIENTER (${pts.length})</div>`;
  pts.forEach(p=>{
    const allTasks = getAllTasks(p);
    const done = allTasks.filter(t=>t.done).length;
    const missing = allTasks.filter(t=>!t.done&&t.enabled!==false);
    const prios = (p.sections.prioritering?.items||[]).filter(x=>!x.done);
    const pct = allTasks.length ? Math.round(done/allTasks.length*100) : 0;
    const tColors = {A:'#3b82f6',B:'#a855f7',C:'#f97316',Obs10:'#ef4444',Obs5:'#6366f1'};
    const tBg = p.team ? `rgba(${p.team==='A'?'59,130,246':p.team==='B'?'168,85,247':p.team==='C'?'249,115,22':p.team==='Obs10'?'239,68,68':'99,102,241'},.15)` : 'var(--s3)';
    html += `<div style="background:var(--s2);border:1px solid var(--border);border-radius:10px;padding:10px 14px;margin-bottom:8px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">
        <span style="font-size:14px;font-weight:700">${p.name}</span>
        ${p.team?`<span style="font-size:10px;font-weight:700;padding:1px 7px;border-radius:5px;background:${tBg};color:${tColors[p.team]||'var(--muted)'}">${TEAM_LABELS[p.team]||p.team}</span>`:''}
        <span style="font-size:11px;color:var(--muted)">${p.room||'Ingen stue'}${p.diag?' · '+p.diag:''}</span>
        <span style="margin-left:auto;font-size:12px;font-weight:700;color:${pct===100?'var(--green)':pct>50?'var(--yellow)':'var(--muted)'}">${pct}% udført</span>
      </div>
      ${prios.length?`<div style="font-size:11px;color:var(--red);font-weight:600;margin-bottom:4px">⚡ ${prios.length} åben${prios.length>1?'e':''} prioritet${prios.length>1?'er':''}</div>`:''}
      ${missing.length?`<div style="display:flex;flex-wrap:wrap;gap:3px">
        ${missing.slice(0,6).map(t=>`<span style="font-size:10px;background:var(--rbg);color:var(--red);border-radius:4px;padding:1px 7px">✗ ${t.label}</span>`).join('')}
        ${missing.length>6?`<span style="font-size:10px;color:var(--muted)">+${missing.length-6} mere</span>`:''}
      </div>`:`<span style="font-size:11px;color:var(--green);font-weight:600">✓ Alle opgaver udført</span>`}
    </div>`;
  });

  // Statistik
  const totalDone = pts.reduce((n,p)=>n+getAllTasks(p).filter(t=>t.done).length,0);
  const totalAll = pts.reduce((n,p)=>n+getAllTasks(p).filter(t=>t.enabled!==false).length,0);
  html += `<div style="background:var(--s2);border-radius:10px;padding:12px;display:flex;gap:20px;flex-wrap:wrap;margin-top:8px">
    <div style="text-align:center"><div style="font-size:22px;font-weight:800;color:var(--green)">${totalAll?Math.round(totalDone/totalAll*100):0}%</div><div style="font-size:10px;color:var(--muted)">Udført</div></div>
    <div style="text-align:center"><div style="font-size:22px;font-weight:800">${totalDone}/${totalAll}</div><div style="font-size:10px;color:var(--muted)">Opgaver</div></div>
    <div style="text-align:center"><div style="font-size:22px;font-weight:800;color:${allPrios.length?'var(--red)':'var(--green)'}">${allPrios.length}</div><div style="font-size:10px;color:var(--muted)">Åbne prioriteter</div></div>
    <div style="text-align:center"><div style="font-size:22px;font-weight:800">${pts.length}</div><div style="font-size:10px;color:var(--muted)">Patienter</div></div>
  </div>`;

  container.innerHTML = html;
}

function printVagtovl(){
  const note = document.getElementById('vagtovlNote')?.value||'';
  const content = document.getElementById('vagtovlContent')?.innerHTML||'';
  const dato = document.getElementById('vagtovlDato')?.textContent||'';
  const win = window.open('','_blank');
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Vagtoverlevering</title>
  <style>body{font-family:sans-serif;padding:20px;color:#111;max-width:800px;margin:0 auto}
  h1{font-size:18px;margin-bottom:4px}.sub{color:#666;font-size:12px;margin-bottom:16px}
  @media print{body{padding:0}}</style></head><body>
  <h1>🌙 Vagtoverlevering</h1><div class="sub">${dato}</div>
  ${content}${note?`<div style="border:1px solid #ddd;border-radius:8px;padding:12px;margin-top:16px;white-space:pre-wrap;font-size:12px"><strong>📝 Vagtnotes:</strong><br>${note}</div>`:''}
  <script>window.onload=()=>window.print();<\/script></body></html>`);
  win.document.close();
}

// ═══════════════════════════════════════════════
// ── COCKPIT / VITAL OVERBLIK ──
// ═══════════════════════════════════════════════
// Cockpit state
let _cockpitSort = 'prio'; // 'prio' | 'name' | 'room' | 'pct'
let _cockpitSelected = new Set();

function openCockpit(){
  _cockpitSelected = new Set();
  document.getElementById('cockpitOverlay').style.display = 'flex';
  renderCockpit();
}
function closeCockpit(){
  document.getElementById('cockpitOverlay').style.display = 'none';
}

function setCockpitSort(s){
  _cockpitSort = s;
  // Update sort button styles
  ['prio','name','room','pct'].forEach(k=>{
    const b = document.getElementById('csort-'+k);
    if(b) b.style.background = k===s ? 'var(--blue)' : 'var(--s3)';
    if(b) b.style.color = k===s ? '#fff' : 'var(--muted2)';
  });
  renderCockpit();
}

function toggleCockpitSelect(pid){
  if(_cockpitSelected.has(pid)) _cockpitSelected.delete(pid);
  else _cockpitSelected.add(pid);
  renderCockpit();
}

function cockpitSelectAll(){
  patients.forEach(p => _cockpitSelected.add(p.id));
  renderCockpit();
}

function cockpitDeleteSelected(){
  if(!_cockpitSelected.size) return;
  if(!confirm(`Slet ${_cockpitSelected.size} patient(er)? Dette kan ikke fortrydes.`)) return;
  patients = patients.filter(p => !_cockpitSelected.has(p.id));
  _cockpitSelected = new Set();
  save();
  render();
  renderCockpit();
...
function renderCockpit(){
  ...
  if(!container) return;
  if(!patients.length){ container.innerHTML='<div style="color:var(--muted);text-align:center;padding:30px">Ingen patienter oprettet</div>'; return; }
  ...
  // Build patient data with today's stats
  const ptData = patients.map(p=>{
    const allTasks = getAllTasks(p);
    const done = allTasks.filter(t=>t.done).length;
    const total = allTasks.filter(t=>t.enabled!==false).length;
    const pct = total ? Math.round(done/total*100) : 0;
    const prios = (p.sections.prioritering?.items||[]).filter(x=>!x.done);
    const p1cnt = prios.filter(x=>x.priority==='red').length;
    const p2cnt = prios.filter(x=>x.priority==='yellow').length;

    // Today's alerts
    const alerts = [];
    const toks = p.sections.ankomst?.items?.find(x=>x.label==='TOKS');
    if(toks && !toks.done) alerts.push({type:'toks',msg:'TOKS ikke udført'});
    (p.sections.medicin?.items||[]).forEach(med=>{
      const overdue = (med.times||[]).filter(t=>{
        const [h,m] = (t.time||'00:00').split(':').map(Number);
        return h*60+m < nowMin && t.status !== 'given';
      });
      if(overdue.length) alerts.push({type:'med',msg:`${med.name||'Medicin'} ikke givet (${overdue.length})`});
    });
    prios.filter(x=>x.priority==='red').forEach(item=>{
      alerts.push({type:'p1',msg:item.label});
    });

    return {p, done, total, pct, prios, p1cnt, p2cnt, alerts};
  });

  // Sort
  const sorted = [...ptData].sort((a,b)=>{
    if(_cockpitSort==='prio') return (b.p1cnt*10+b.p2cnt+b.alerts.length) - (a.p1cnt*10+a.p2cnt+a.alerts.length);
    if(_cockpitSort==='pct')  return a.pct - b.pct;
    if(_cockpitSort==='name') return (a.p.name||'').localeCompare(b.p.name||'','da');
    if(_cockpitSort==='room') return (a.p.room||'').localeCompare(b.p.room||'','da',{numeric:true});
    return 0;
  });

  const allAlerts = ptData.flatMap(d=>d.alerts.map(a=>({...a,pid:d.p.id,name:d.p.name,room:d.p.room,team:d.p.team})));
  const allSelected = patients.length > 0 && patients.every(p=>_cockpitSelected.has(p.id));

  let html = `
  <!-- Dato og sortering header -->
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:14px">
    <div>
      <div style="font-size:13px;font-weight:800;color:var(--text)">📅 ${todayStr}</div>
      <div style="font-size:11px;color:var(--muted)">Viser kun dagens status — opdateres i realtid</div>
    </div>
    <div style="display:flex;gap:5px;align-items:center;flex-wrap:wrap">
      <span style="font-size:10px;color:var(--muted);font-weight:700">Sorter:</span>
      <button id="csort-prio" onclick="setCockpitSort('prio')" style="font-size:10px;padding:3px 8px;border-radius:6px;border:1px solid var(--border2);cursor:pointer;font-family:inherit;background:var(--blue);color:#fff">⚡ Prioritet</button>
      <button id="csort-pct" onclick="setCockpitSort('pct')" style="font-size:10px;padding:3px 8px;border-radius:6px;border:1px solid var(--border2);cursor:pointer;font-family:inherit;background:var(--s3);color:var(--muted2)">% Udført</button>
      <button id="csort-room" onclick="setCockpitSort('room')" style="font-size:10px;padding:3px 8px;border-radius:6px;border:1px solid var(--border2);cursor:pointer;font-family:inherit;background:var(--s3);color:var(--muted2)">Stue</button>
      <button id="csort-name" onclick="setCockpitSort('name')" style="font-size:10px;padding:3px 8px;border-radius:6px;border:1px solid var(--border2);cursor:pointer;font-family:inherit;background:var(--s3);color:var(--muted2)">Navn</button>
    </div>
  </div>`;

  // Alerts
  if(allAlerts.length){
    const typeIcons = {toks:'🩺',med:'💊',p1:'⚡'};
    const typeBg = {toks:'var(--ybg)',med:'var(--obg,rgba(251,146,60,.12))',p1:'var(--rbg)'};
    const typeColor = {toks:'var(--yellow)',med:'var(--orange,#f97316)',p1:'var(--red)'};
    html += `<div style="margin-bottom:16px">
      <div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--red);margin-bottom:8px;letter-spacing:.5px">🚨 KRÆVER OPMÆRKSOMHED I DAG (${allAlerts.length})</div>`;
    allAlerts.forEach(a=>{
      html += `<div style="background:${typeBg[a.type]};border-radius:8px;padding:7px 12px;margin-bottom:4px;display:flex;align-items:center;gap:10px;cursor:pointer" onclick="closeCockpit();setActivePatient('${a.pid}')">
        <span style="font-size:13px">${typeIcons[a.type]}</span>
        <div style="flex:1"><span style="font-size:12px;font-weight:700;color:${typeColor[a.type]}">${a.msg}</span>
        <span style="font-size:11px;color:var(--muted);margin-left:8px">${a.name}${a.room?' · '+a.room:''}</span></div>
        ${a.team?`<span style="font-size:10px;font-weight:700;color:${tColors[a.team]||'var(--muted)'}">${TEAM_LABELS[a.team]||a.team}</span>`:''}
      </div>`;
    });
    html += '</div>';
  } else {
    html += `<div style="background:var(--gbg);border:1px solid var(--gbrd);border-radius:10px;padding:10px;margin-bottom:14px;text-align:center;color:var(--green);font-weight:700;font-size:13px">✓ Ingen akutte alerts i dag</div>`;
  }

  // Bulk delete bar
  html += `<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap">
    <div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--muted);letter-spacing:.5px">🏥 PATIENTER (${patients.length})</div>
    <div style="margin-left:auto;display:flex;gap:6px;align-items:center">
      <button onclick="cockpitSelectAll()" style="font-size:11px;padding:3px 9px;border-radius:6px;border:1px solid var(--border2);background:var(--s3);color:var(--muted2);cursor:pointer;font-family:inherit">
        ${allSelected?'☑':'☐'} Vælg alle
      </button>
      ${_cockpitSelected.size ? `<button onclick="cockpitDeleteSelected()" style="font-size:11px;padding:3px 9px;border-radius:6px;border:1px solid var(--rbrd);background:var(--rbg);color:var(--red);cursor:pointer;font-family:inherit;font-weight:700">
        🗑 Slet valgte (${_cockpitSelected.size})
      </button>` : ''}
    </div>
  </div>
  <div style="display:grid;gap:6px">`;

  sorted.forEach(({p,done,total,pct,prios,p1cnt,p2cnt,alerts:ptAlerts})=>{
    const barColor = pct===100?'var(--green)':pct>66?'var(--blue)':pct>33?'var(--yellow)':'var(--red)';
    const isSelected = _cockpitSelected.has(p.id);
    html += `<div style="background:var(--s2);border:1.5px solid ${isSelected?'var(--blue)':ptAlerts.length?'var(--rbrd,rgba(239,68,68,.3))':'var(--border)'};border-radius:10px;padding:10px 12px;display:flex;align-items:center;gap:8px">
      <!-- Checkbox -->
      <div onclick="toggleCockpitSelect('${p.id}')" style="cursor:pointer;font-size:16px;flex-shrink:0;color:${isSelected?'var(--blue)':'var(--border2)'}">
        ${isSelected?'☑':'☐'}
      </div>
      <!-- Content -->
      <div style="flex:1;min-width:0;cursor:pointer" onclick="closeCockpit();setActivePatient('${p.id}')">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:5px;flex-wrap:wrap">
          ...
          <span style="font-size:11px;color:var(--muted)">${p.room||'—'}</span>
          ${p.team?`<span style="font-size:10px;font-weight:700;color:${tColors[p.team]||'var(--muted)'}">${TEAM_LABELS[p.team]||p.team}</span>`:''}
          ${p1cnt?`<span style="font-size:10px;font-weight:800;background:var(--rbg);color:var(--red);border-radius:4px;padding:1px 5px">🚨 ${p1cnt} P1</span>`:''}
          ${p2cnt&&!p1cnt?`<span style="font-size:10px;font-weight:800;background:var(--ybg);color:var(--yellow);border-radius:4px;padding:1px 5px">⚡ ${p2cnt} P2</span>`:''}
          <span style="margin-left:auto;font-size:11px;font-weight:700;color:${barColor}">${pct}%</span>
        </div>
        <div style="display:flex;align-items:center;gap:8px">
          <div style="flex:1;height:5px;background:var(--s3);border-radius:3px;overflow:hidden">
            <div style="height:100%;width:${pct}%;background:${barColor};border-radius:3px"></div>
          </div>
          <span style="font-size:10px;color:var(--muted);min-width:32px;text-align:right">${done}/${total}</span>
        </div>
        ${ptAlerts.length?`<div style="margin-top:5px;display:flex;flex-wrap:wrap;gap:3px">
          ${ptAlerts.slice(0,3).map(a=>`<span style="font-size:9px;background:var(--rbg);color:var(--red);border-radius:4px;padding:1px 6px">${a.msg}</span>`).join('')}
          ${ptAlerts.length>3?`<span style="font-size:9px;color:var(--muted)">+${ptAlerts.length-3} mere</span>`:''}
        </div>`:''}
      </div>
      <!-- Slet enkeltvis -->
      <button onclick="cockpitDeleteOne('${p.id}')" title="Slet patient"
        style="flex-shrink:0;padding:4px 7px;border-radius:6px;border:1px solid var(--rbrd);background:var(--rbg);color:var(--red);cursor:pointer;font-family:inherit;font-size:13px">
        🗑
      </button>
    </div>`;
  });
...
// ── VAGTNOTEPAD ──
// ═══════════════════════════════════════════════
function toggleVagtnotepad(){
  const bar = document.getElementById('vagtnotepadBar');
  if(!bar) return;
  const isOpen = bar.style.display !== 'none';
  bar.style.display = isOpen ? 'none' : 'block';
  if(!isOpen){
    const saved = localStorage.getItem('kp2_vagtnote')||'';
    const ta = document.getElementById('vagtnotepadText');
    if(ta){ ta.value = saved; ta.focus(); }
    updateVagtnoteReset();
  }
}

function saveVagtnote(){
  const ta = document.getElementById('vagtnotepadText');
  if(ta) localStorage.setItem('kp2_vagtnote', ta.value);
}

function updateVagtnoteReset(){
  const info = document.getElementById('vagtnoteResetInfo');
  if(!info) return;
  const now = new Date();
  const h = now.getHours();
  const shifts = [7,15,23];
  const nextShift = shifts.find(s=>s>h) || shifts[0];
  info.textContent = `Nulstilles kl. ${String(nextShift).padStart(2,'0')}:00`;
}

// Auto-nulstil notepad ved vagtskifte (07, 15, 23)
setInterval(()=>{
  const now = new Date();
  const h = now.getHours(), m = now.getMinutes(), s = now.getSeconds();
  if((h===7||h===15||h===23) && m===0 && s<10){
    const last = localStorage.getItem('kp2_vagtnote_reset')||'';
    const key = `${h}:${now.toLocaleDateString()}`;
    if(last !== key){
      localStorage.setItem('kp2_vagtnote','');
      localStorage.setItem('kp2_vagtnote_reset', key);
      const ta = document.getElementById('vagtnotepadText');
      if(ta) ta.value = '';
      showPrioToast('Vagtnotepad er nulstillet til ny vagt', 'yellow');
    }
  }
}, 8000);

// ═══════════════════════════════════════════════
// ── DATO-NULSTILLING VED MIDNAT ──
// ═══════════════════════════════════════════════
function checkMidnightReset(){
  const today = new Date().toLocaleDateString('da-DK');
  const lastReset = localStorage.getItem('kp2_last_reset');
  if(lastReset === today) return; // allerede nulstillet i dag
  localStorage.setItem('kp2_last_reset', today);
  let changed = 0;
  patients.forEach(p=>{
    Object.values(p.sections).forEach(sec=>{
      if(!sec.tasks) return;
      sec.tasks.forEach(t=>{
        if(t.done && t.resetDaily !== false){
          t.done = false;
          t.status = 'pending';
          t.doneAt = '';
          // Nulstil tidsslots
          if(t.times) t.times = t.times.map(ts=>({...(typeof ts==='object'?ts:{time:ts}), status:'pending', dt:''}));
          changed++;
        }
      });
    });
    // Nulstil medicin-givet status
    (p.sections.medicin?.items||[]).forEach(med=>{
      med.times = (med.times||[]).map(ts=>({...ts, status:'pending', givetAt:'', givetAf:''}));
    });
  });
  if(changed > 0){
    save();
    showPrioToast(`Ny dag — ${changed} opgaver nulstillet til Afventer`, 'yellow');
  }
}

// Tjek ved opstart og hvert minut
checkMidnightReset();
...
// ── MEDICINSKEMA PRINT ──
// ═══════════════════════════════════════════════
function printMedSkema(){
  const dato = new Date().toLocaleDateString('da-DK',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
  let rows = '';
  patients.forEach(p=>{
    const meds = p.sections.medicin?.items||[];
    if(!meds.length) return;
    rows += `<tr style="background:#f8f9fa"><td colspan="5" style="font-weight:bold;padding:8px 10px;border-bottom:2px solid #333">${p.name} — ${p.room||'Ingen stue'}${p.diag?' ('+p.diag+')':''}</td></tr>`;
    meds.forEach(med=>{
      const times = (med.times||[]);
      rows += `<tr>
        <td style="padding:6px 10px">${med.name||'Medicin'}</td>
        <td style="padding:6px 10px">${med.dose||'—'}</td>
        <td style="padding:6px 10px">${med.freqLabel||'—'}</td>
        <td style="padding:6px 10px">${times.map(t=>t.time||t).join(', ')||'—'}</td>
        <td style="padding:6px 10px">${times.filter(t=>t.status==='given').length}/${times.length} givet</td>
      </tr>`;
    });
  });
  const win = window.open('','_blank');
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Medicinskema ${dato}</title>
  ...
  th{background:#333;color:#fff}@media print{body{padding:0}}</style></head><body>
  <h2 style="margin-bottom:4px">💊 Medicinskema</h2>
  <p style="color:#666;margin-bottom:16px">${dato} · Neurologi/Stroke · Klinisk Plejeplan</p>
  <table><thead><tr><th>Præparat</th><th>Dosis</th><th>Frekvens</th><th>Tider</th><th>Givet</th></tr></thead>
  <tbody>${rows}</tbody></table>
  <script>window.onload=()=>window.print();<\/script></body></html>`);
  win.document.close();
}

// ── AUTO-OPRET STANDARD AFDELING VED FØRSTE KØRSEL ──
// Kører kun én gang — opretter "neurologi-n" hvis ingen afdelinger findes
async function ensureDefaultDept(){
  try {
    const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
    const res = await fetch(`${base}/system/afdelinger.json`);
    const depts = res.ok ? await res.json() : null;
    if(!depts || Object.keys(depts).length === 0){
      // Opret standard-afdeling
      await fetch(`${base}/system/afdelinger/neurologi-n.json`, {
        method: 'PUT',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({
          name: 'Neurologisk Afdeling N',
          code: 'NEURO-N-2025',
          active: true,
          dbSecret: 'dept-' + Math.random().toString(36).slice(2,14) + Math.random().toString(36).slice(2,14),
          createdAt: new Date().toLocaleDateString('da-DK')
        })
      });
      console.log('Standard afdeling oprettet');
    }
  } catch(e){ console.warn('ensureDefaultDept fejl:', e); }
}
ensureDefaultDept();
