...
// ═══════════════════════════════════════════════════════
// ── AFDELING / MULTI-DEPARTMENT SYSTEM ──
// ═══════════════════════════════════════════════════════

// Admin-kode (skjult i koden — skift denne til noget unikt)
const ADMIN_CODE = 'KP2-ADMIN-2025';

// Global aktiv afdeling
let _currentDeptId   = null;  // fx 'neurologi-n'
let _currentDeptName = null;  // fx 'Neurologisk Afdeling N'
let _currentDeptCode = null;  // adgangskoden for afdelingen
let _isAdmin         = false;

// Hjælpefunktion: opbyg Firebase-sti for aktiv afdeling
// dbSecret er en uforudsigelig UUID der bruges som den rigtige Firebase-sti
// Den menneskelige ID (neurologi-n) bruges kun til visning
let _currentDeptSecret = null;

function _deptPath(subpath){
  // Brug dbSecret hvis tilgængeligt — ellers menneskelig ID
  const key = _currentDeptSecret || _currentDeptId || 'default';
  return `/afdelinger/${key}/${subpath}`;
}

// ── Indlæs gemte afdelingsoplysninger ──
function loadDeptSession(){
  try {
    const s = JSON.parse(localStorage.getItem('kp2_dept_session')||'null');
    if(s && s.code){
      if(s.isAdmin){
        _currentDeptId     = s.id === '__admin__' ? null : (s.id||null);
        _currentDeptName   = s.name;
        _currentDeptCode   = s.code;
        _currentDeptSecret = s.secret || null;
        _isAdmin           = true;
        return true;
      }
      if(s.id && s.name){
...
function saveDeptSession(){
  localStorage.setItem('kp2_dept_session', JSON.stringify({
    id: _currentDeptId, name: _currentDeptName,
    code: _currentDeptCode, isAdmin: _isAdmin,
    secret: _currentDeptSecret
  }));
}

function clearDeptSession(){
  localStorage.removeItem('kp2_dept_session');
  _currentDeptId = _currentDeptName = _currentDeptCode = null;
  _isAdmin = false;
}

// ── Vis login-skærm ──
function showDeptLogin(message=''){
  document.getElementById('deptLoginOverlay').style.display = 'flex';
  document.getElementById('mainApp').style.display = 'none';
  if(message){
    const el = document.getElementById('deptLoginMsg');
    if(el){ el.textContent = message; el.style.display = 'block'; }
  }
}

function hideDeptLogin(){
  document.getElementById('deptLoginOverlay').style.display = 'none';
  document.getElementById('mainApp').style.display = '';
}

// ── Login-forsøg ──
async function tryDeptLogin(){
  const codeInput = document.getElementById('deptCodeInput');
  const code = codeInput ? codeInput.value.trim() : '';
  if(!code){ showDeptLoginError('Indtast en afdelingskode'); return; }

  const loginBtn = document.getElementById('deptLoginBtn');
  if(loginBtn){ loginBtn.disabled = true; loginBtn.textContent = 'Checker...'; }

  // Tjek admin-kode
  if(code === ADMIN_CODE){
    _currentDeptId   = null; // Admin vælger afdeling først
    _currentDeptName = 'Admin — Vælg afdeling';
    _currentDeptCode = ADMIN_CODE;
    _isAdmin         = true;
    saveDeptSession();
    hideDeptLogin();
    renderAdminDeptHeader();
    // Åbn admin-panel med det samme så de vælger en afdeling
    openAdminDeptPanel();
    return;
  }

  // Tjek afdelingskoder i Firebase
  try {
    ...
    if(depts){
      const match = Object.entries(depts).find(([id, d]) => d.code === code && d.active !== false);
      if(match){
        const [id, dept] = match;
        _currentDeptId     = id;
        _currentDeptName   = dept.name;
        _currentDeptCode   = code;
        _currentDeptSecret = dept.dbSecret || id; // brug secret hvis den findes
        _isAdmin           = false;
        saveDeptSession();
        hideDeptLogin();
        renderDeptHeader();
        await initFirebase();
        await loadDeptDefaults();
        if(loginBtn){ loginBtn.disabled = false; loginBtn.textContent = 'Log ind'; }
        return;
      }
    }
  } catch(e){ console.warn('Dept lookup fejl:', e); }

  showDeptLoginError('Ugyldig kode — kontakt IT-afdelingen');
  if(loginBtn){ loginBtn.disabled = false; loginBtn.textContent = 'Log ind'; }
}

function showDeptLoginError(msg){
  const el = document.getElementById('deptLoginMsg');
  if(el){ el.textContent = '⚠ '+msg; el.style.display = 'block'; el.style.color = 'var(--red)'; }
}

function logoutDept(){
  if(!confirm('Log ud af afdelingen? Du skal bruge koden igen for at logge ind.')) return;
  clearDeptSession();
  patients = [];
  localStorage.removeItem('kp2_pts');
  showDeptLogin();
}

// ── Opdater afdeling-header ──
function renderDeptHeader(){
  const el = document.getElementById('deptNameBadge');
  if(!el) return;
  el.textContent = _currentDeptName || '';
  el.style.display = _currentDeptName ? '' : 'none';
}

function renderAdminDeptHeader(){
  renderDeptHeader();
  const adminBtn = document.getElementById('adminDeptBtn');
  if(adminBtn) adminBtn.style.display = '';
}

// ── Admin: skift aktiv afdeling ──
async function adminSwitchDept(id){
  const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
  const res = await fetch(`${base}/system/afdelinger.json`);
  const depts = res.ok ? await res.json() : {};
  const dept = depts[id];
  if(!dept){ alert('Afdeling ikke fundet'); return; }

  // Stop eksisterende sync og ryd data
  if(typeof _fbListener !== 'undefined' && _fbListener) clearInterval(_fbListener);
  _fbDb = null;
  patients = [];
  localStorage.removeItem('kp2_pts');
  ...
  // Sæt ny afdeling
  _currentDeptId     = id;
  _currentDeptName   = (dept.name || id) + ' (Admin)';
  _currentDeptSecret = dept.dbSecret || id;
  saveDeptSession();

  document.getElementById('adminDeptPanel').style.display = 'none';
  renderDeptHeader();
  render();

  // Start Firebase for den nye afdeling
  await initFirebase();
  await loadDeptDefaults();
  showPrioToast('Skiftet til: ' + (dept.name||id), 'green');
}

// ── Admin panel: vis afdelinger ──
async function openAdminDeptPanel(){
  const panel = document.getElementById('adminDeptPanel');
  if(!panel) return;
  panel.style.display = 'flex';
  await loadAdminDeptPanel();
}

async function loadAdminDeptPanel(){
  const content = document.getElementById('adminDeptContent');
  if(!content) return;
  content.innerHTML = '<div style="color:var(--muted);text-align:center;padding:20px">Henter afdelinger...</div>';
  ...
  try {
    ...
    let html = `<div style="display:flex;justify-content:flex-end;margin-bottom:12px">
      <button class="btn btn-primary btn-sm" onclick="adminAddDept()">+ Tilføj afdeling</button>
    </div>`;

    if(!entries.length){
      html += '<div style="color:var(--muted);text-align:center;padding:20px">Ingen afdelinger oprettet endnu</div>';
    } else {
      entries.forEach(([id, dept]) => {
        const isActive = id === _currentDeptId;
        html += `<div style="background:var(--s2);border:1px solid ${isActive?'var(--blue)':'var(--border)'};border-radius:10px;padding:12px 14px;margin-bottom:8px">
          <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
            <div style="flex:1">
              <div style="font-size:14px;font-weight:700">${dept.name}</div>
              <div style="font-size:11px;color:var(--muted);margin-top:2px">
                ID: <code style="background:var(--s3);padding:1px 5px;border-radius:4px">${id}</code>
                · Kode: <code style="background:var(--s3);padding:1px 5px;border-radius:4px">${dept.code}</code>
                · ${dept.active!==false ? '<span style="color:var(--green)">✓ Aktiv</span>' : '<span style="color:var(--red)">✗ Deaktiveret</span>'}
              </div>
            </div>
            <div style="display:flex;gap:5px">
              <button class="btn btn-ghost btn-sm" onclick="adminSwitchDept('${id}')" title="Skift til denne afdeling">👁 Vis</button>
              <button class="btn btn-ghost btn-sm" onclick="adminEditDept('${id}','${dept.name}','${dept.code}',${dept.active!==false})" title="Rediger">✎ Rediger</button>
              <button class="btn btn-sm" style="background:var(--rbg);color:var(--red);border:1px solid var(--rbrd);font-size:11px"
                onclick="adminDeleteDept('${id}','${dept.name}')">🗑</button>
            </div>
          </div>
        </div>`;
      });
    ...
    content.innerHTML = `<div style="color:var(--red)">Fejl: ${e.message}</div>`;
  }
}

async function adminAddDept(){
  const name = prompt('Afdelingens navn:\n(fx "Neurologisk Afdeling N")');
  if(!name) return;
  const id = prompt('Afdelings-ID (kun bogstaver og bindestreg):\n(fx "neurologi-n")');
  if(!id || !/^[a-z0-9\-]+$/.test(id)){ alert('Ugyldigt ID — brug kun små bogstaver og bindestreger'); return; }
  const code = prompt('Adgangskode for afdelingen:\n(personalet bruger denne kode til login)');
  if(!code){ alert('Kode skal udfyldes'); return; }

  // Generer unik hemmelig database-sti (UUID) — kan ikke gættes
  const dbSecret = 'dept-' + crypto.randomUUID().replace(/-/g,'').slice(0,24);

  const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
  await fetch(`${base}/system/afdelinger/${id}.json`, {
    method:'PUT', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({
      name, code, active:true,
      dbSecret, // hemmelig sti i Firebase
      createdAt: new Date().toLocaleDateString('da-DK')
    })
  });
  await loadAdminDeptPanel();
  showPrioToast(`Afdeling "${name}" oprettet`, 'green');
}

async function adminEditDept(id, currentName, currentCode, currentActive){
  const name = prompt('Nyt navn:', currentName);
  if(name === null) return;
  const code = prompt('Ny kode:', currentCode);
  if(code === null) return;
  const active = confirm('Er afdelingen aktiv?\n(OK = Ja, Annuller = Deaktiveret)');

  const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
  await fetch(`${base}/system/afdelinger/${id}.json`, {
    method:'PATCH', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ name: name||currentName, code: code||currentCode, active })
  });
  await loadAdminDeptPanel();
  showPrioToast('Afdeling opdateret', 'green');
}

async function adminDeleteDept(id, name){
  if(!confirm(`Slet afdelingen "${name}"?\n\nDette sletter IKKE patientdata — kun adgangen til afdelingen.`)) return;
  const base = 'https://patientertjek-default-rtdb.europe-west1.firebasedatabase.app';
  await fetch(`${base}/system/afdelinger/${id}.json`, { method:'DELETE' });
  await loadAdminDeptPanel();
  showPrioToast(`Afdeling "${name}" slettet`, 'yellow');
}
...
async function initFirebase(){
  ...
  try {
    const res = await fetch(_fbDb.url(_deptPath('ping')), {
      method: 'PUT',
  ...
  try {
    const res = await fetch(_fbDb.url(_deptPath('patients')), {cache:'no-store'});
    if(res.ok){
  ...
  window.addEventListener('beforeunload', ()=>{
    if(_fbDb && patients.length > 0){
      try {
        ...
        xhr.open('PUT', _fbDb.url(_deptPath('patients')), false);
...
async function pushToCloud(){
  ...
  try {
    _ignoreNext = true;
    const res = await fetch(_fbDb.url(_deptPath('patients')), {
      method: 'PUT',
      headers: {'Content-Type':'application/json'},
...
async function saveBackupToFirebase(){
  ...
  try {
    ...
    await fetch(_fbDb.url(_deptPath('backups/'+key)), {
      method: 'PUT',
      headers: {'Content-Type':'application/json'},
...
async function logActivity(action, detail, pid){
  ...
  try {
    await fetch(_fbDb.url(_deptPath('log/'+Date.now()+'_'+Math.random().toString(36).slice(2,6))), {
      method: 'PUT',
      headers: {'Content-Type':'application/json'},
...
async function loadActivityLog(){
  ...
  try {
    const res = await fetch(_fbDb.url(_deptPath('log'), {orderBy:'"ts"', limitToLast:100}));
...
async function loadBackupList(){
  ...
  try {
    const res = await fetch(_fbDb.url(_deptPath('backups')));
    const data = await res.json();
...
async function restoreBackup(key){
  ...
  try {
    const res = await fetch(_fbDb.url(_deptPath('backups/'+key)));
...
async function loadArchive(){
  ...
  if(!_fbDb){ content.innerHTML = '<div style="color:var(--muted);padding:10px">Kræver Firebase</div>'; return; }
  try {
    const res = await fetch(_fbDb.url(_deptPath('archive')));
    const data = await res.json();
...
async function restoreArchivedPatient(key){
  if(!_fbDb) return;
  const res = await fetch(_fbDb.url(_deptPath('archive/'+key)));
...
function logActivity(action, detail, pid){
  ...
  fetch(_fbDb.url(_deptPath('log/'+Date.now()+'_'+Math.random().toString(36).slice(2,6))), {
...
async function testSyncConnection(){
  ...
  try {
    const putRes = await fetch(`${base}/afdelinger/${_currentDeptId||'default'}/ping.json`, {
      method:'PUT', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ts: Date.now(), ok: true})
    });
    ...
    const getRes = await fetch(`${base}/afdelinger/${_currentDeptId||'default'}/ping.json`, {cache:'no-store'});
...
function startSync(){
  // Kun start sync hvis afdelingssession er aktiv
...
function deleteSelected(){
  ...
  toDelete.forEach(p => {
    if(_fbDb) fetch(_fbDb.url(_deptPath('archive/'+p.id)), {method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...p,archivedAt:fmtDT(Date.now()),archivedBy:_actorName||'Ukendt'})}).catch(()=>{});
...
function togglePt(pid){
  const p = patients.find(x=>x.id===pid);
  if(!p) return;
  p.open = !p.open;
  const card = document.getElementById('ptcard-'+pid);
  if(card) card.className = 'pt-card' + (p.open?' open':'');
  // Gem open-state uden at pushe til Firebase (lokal præference)
  localStorage.setItem('kp2_pts', JSON.stringify(patients));
}

function collapseAllCards(){
  patients.forEach(p=>{ p.open = false; });
  localStorage.setItem('kp2_pts', JSON.stringify(patients));
  document.querySelectorAll('.pt-card').forEach(c=>c.classList.remove('open'));
}

function expandAllCards(){
  patients.forEach(p=>{ p.open = true; });
  localStorage.setItem('kp2_pts', JSON.stringify(patients));
