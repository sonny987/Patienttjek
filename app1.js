...
function _deptPath(subpath){
  const dept = _currentDeptId || 'default';
  return `/afdelinger/${dept}/${subpath}`;
}
...
function loadDeptSession(){
  try {
    ...
    if(s && s.code){
      // Admin-session: sæt isAdmin men ikke dept-id (de vælger afdeling)
      if(s.isAdmin){
        _currentDeptId   = s.id === '__admin__' ? null : s.id;
...
async function adminAddDept(){
  ...
  await fetch(`${base}/system/afdelinger/${id}.json`, {
    ...
    body: JSON.stringify({ name, code, active:true, createdAt: new Date().toLocaleDateString('da-DK') })
  });
  await loadAdminDeptPanel();
