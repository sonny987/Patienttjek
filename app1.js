...
function toggleBrightnessPanel(){
  ...
  if(!isOpen){
    // Restore saved value
    const saved = parseInt(localStorage.getItem('kp2_brightness')||'0');
...
// Mørke farver (0%) og lyse farver (100%)
const THEME_DARK = {
  '--bg':      [10,13,20],
  '--s1':      [17,21,32],
  '--s2':      [22,27,46],
...
function applyBrightness(val){
  val = parseInt(val);
  localStorage.setItem('kp2_brightness', val);
  const t = val / 100; // 0 = mørk, 1 = lys
  const root = document.documentElement;

  // Interpoler alle CSS custom properties
  Object.keys(THEME_DARK).forEach(key => {
    const dark  = THEME_DARK[key];
    const light = THEME_LIGHT[key];
  ...
  // Fjern light-mode klasse — vi styrer alt via CSS vars nu
  document.body.classList.remove('light-mode');
  document.body.style.filter = '';

  updateBrightnessLabel(val);
  const btn = document.getElementById('themeBtn');
  if(btn) btn.textContent = val < 30 ? '🌙' : val < 70 ? '🌤️' : '☀️';
}

function updateBrightnessLabel(val){
  const lbl = document.getElementById('brightnessLabel');
  if(!lbl) return;
  if(val <= 0) lbl.textContent = 'Mørk';
  else if(val <= 25) lbl.textContent = 'Svagt mørk';
  else if(val <= 50) lbl.textContent = 'Halvlys';
  else if(val <= 75) lbl.textContent = 'Lys';
  else lbl.textContent = 'Fuldt lys';
}

// Luk brightness panel når man klikker udenfor
...
function renderPriorityView(){
  ...
  ['A','B','C','Obs10','Obs5'].forEach(t => {
    ...
    el.innerHTML = cnt > 0 ? `Team ${t} <span class="team-badge badge-${t.toLowerCase()}">${cnt}</span>` : `Team ${t}`;
  });
